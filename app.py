...
def init_db():
    ...
    """)
    # Migration: add is_pinned (separate connection so failure is isolated)
    conn.commit()
    try:
        ...
        conn.rollback()
    # Migration: deduplicate rows then add unique index
    try:
        cur.execute("""
...
def _build_clean_history(history):
    """Convert raw session messages into a clean list for the AI.
    Handles image HTML, image analysis results, uploaded image labels,
    and edited image markers — so the AI always gets readable context
    regardless of what happened in the chat."""
    last_img_prompt   = None
    last_img_analysis = None
    clean_msgs        = []

    for msg in history[-30:]:  # increased from 20 → 30 so brain sees more context
        txt = msg["text"]

        # ── Generated image (HTML blob) ──
        if 'jarvis-img-wrap' in txt:
            ...
            clean_msgs.append({
                "role":    "assistant",
                "content": f"[I generated an image with this description: {img_prompt}]"
            })

        # ── Background / on-demand vision analysis result ──
            ...
            # Inject into history as a system note so the AI can reference it naturally
            clean_msgs.append({
                "role":    "system",
                "content": (
                    "[INTERNAL — image visual description, do NOT repeat verbatim to the user unless asked]: "
                    + analysis_text
                )
            })

        # ── Edited image marker from NVIDIA ──
        elif txt.startswith("__EDITED_IMAGE__"):
            clean_msgs.append({
                "role":    "assistant",
                "content": "[I edited the image as requested and displayed the result.]"
            })

        # ── Uploaded image label e.g. "[image uploaded] caption" ──
...
def ask_jarvis_brain(prompt, history=[]):
    """Main AI brain — reads full chat history and decides what action to take.
    Returns a dict: {action, reply, image_prompt, high_quality, wants_code}
    action is one of: 'text' | 'generate_image' | 'edit_image'
    Falls back to plain text reply on any JSON parse failure."""
...
YOU MUST ALWAYS RESPOND IN THIS EXACT JSON FORMAT — no markdown, no explanation outside the JSON:
{
  ...
  "wants_code": true | false
    ...
    # Inject last image context into system if applicable
    if last_img_prompt:
        system_msg += (
            f"\n\nLAST GENERATED IMAGE: The most recent image in this chat was: \"{last_img_prompt}\". "
            f"If the user asks to change/edit/redo it, use action='edit_image' and write a complete new prompt combining the original with their requested changes."
        )
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 1:
                # Full fallback — return whatever the model said as plain text
                try:
...
def ask_jarvis(prompt, history=[], wants_code=False):
    """Legacy plain-text path — kept for math fallback and any direct callers.
    Uses the same clean history builder so context is consistent."""
    API_KEY = os.environ["GROQ_API_KEY"]
    url     = "https://api.groq.com/openai/v1/chat/completions"
    headers = {"Authorization": "Bearer " + API_KEY, "Content-Type": "application/json"}
...
4. You have no knowledge of any verification codes. Never guess or invent them."""
    ...
    if last_img_analysis:
        final_system += (
            "\n\nHIDDEN IMAGE CONTEXT (internal use only): "
            + last_img_analysis
        )
    if last_img_prompt:
        final_system += (
            f"\n\nLAST IMAGE: \"{last_img_prompt}\". "
            f"If the user asks to change/edit it respond ONLY as: ##IMAGE:<full new prompt>##"
        )
...
def _build_reply(user_msg):
    ...
    Priority order:
      1. Owner code verification (stateful — must stay first)
      2. Time / date shortcuts (instant, no AI needed)
      3. Math expression (safe_eval — no AI needed)
      4. 'history' keyword
      5. Brain AI decision for everything else (text, generate_image, edit_image)
    """
    reply = ""

    # ── 1. Owner code verification ──
    ...
    if (any(op in user_msg for op in ["+", "-", "*", "/", "×", "÷"])
        ...
        try:
            ...
            pass  # Not a valid math expression — fall through to brain

    # ── 5. Bad word alert (fire-and-forget, doesn't affect reply) ──
    if contains_bad_words(user_msg):
        send_discord_alert(session["user"], "Abusive language toward Jarvis or Aadin", user_msg)

    # ── 6. If user is asking about a generated image, wait for vision analysis ──
    # The background thread may still be running. Poll up to 8s so the brain
    # gets real pixel data instead of hallucinating the answer.
    ...
    _has_recent_image = any(
        'jarvis-img-wrap' in m.get("text", "")
        for m in session.get("messages", [])
    )
    _is_img_question = _has_recent_image and any(
        t in user_msg.lower() for t in _img_question_triggers
    )
    if _is_img_question:
        _waited = 0
        while _waited < 8:
            _has_analysis = any(
                m.get("text", "").startswith("[IMAGE ANALYSIS RESULT]")
                for m in session.get("messages", [])
            )
            if _has_analysis:
                break
    ...
    # ── 7. Brain AI — decides action from full chat context ──
    ...
    if action == "text":
        ...
        # Security / owner special tokens (brain embeds them in reply field)
        ...
        # If brain fell back to raw non-JSON text, check for legacy ##IMAGE:## token
        # (brain shouldn't produce this but guard just in case)
    ...
    if action in ("generate_image", "edit_image"):
        ...
        if not img_prompt:
            # Brain gave generate_image but no prompt — ask user to clarify
        ...
        # Background vision analysis so follow-up questions work
        threading.Thread(
            target=_auto_analyse_generated_image,
            args=(img_prompt, session["messages"], _img_src),
            daemon=True
        ).start()
        return reply
...
def flush_session():
    """Called by navigator.sendBeacon on tab close — saves session to DB immediately."""
    _flush_chat_session()
    return _json_mod.dumps({"ok": True}), 200, {"Content-Type": "application/json"}
...
def share_chat():
    ...
    # Merge session messages with DB-saved messages so shared chat is complete.
    # Session holds latest unsaved messages; DB holds the periodically-saved full copy.
    ...
    try:
        ...
        for row in db_rows:
            if row["session_key"] == current_key:
                ...
                break
        # Use whichever is longer — session has latest, DB has earlier history
