...
def _compact_for_session(text):
    # Strip YouTube card tag before saving to session history
    text = re.sub(r'\s*\[YOUTUBE:[^\]]*\]', '', text).strip()
...
def _build_clean_history(history):
    ...
    clean_msgs        = []
    # llama-3.1-8b-instant context ~8k tokens. System prompt + new message + response reserves ~2k tokens.
    # Budget ~6k tokens = ~24k chars. Use 12k conservatively to leave headroom for long replies.
    ...
    # Use last 20 messages max, each capped at 300 chars, so worst case ~6k chars
    for msg in history[-20:]:
        ...
        if txt.startswith("[IMAGE GENERATED:"):
            ...
            role = "user" if msg["sender"] == "You" else "assistant"
            # Cap individual messages at 300 chars — prevents a long recipe from consuming all budget
            content = txt[:300] if len(txt) > 300 else txt
...
GITHUB / SOURCE CODE RULES:
- Rakai is fully open-source! The GitHub repo is: https://github.com/heyitsaadin/Rekai-ai
- If a user asks how you were built, how Aadin made you, can they see the code, or anything about your source code → enthusiastically share the GitHub link and invite them to explore it.
- Example reply: "Aadin open-sourced me! 🎉 You can see the full code here: https://github.com/heyitsaadin/Rekai-ai — feel free to explore, star it, or ask me anything about how I work! ⭐"

YOUTUBE VIDEO CARD RULES:
- If the user asks for a video, tutorial, or mentions wanting to watch/learn something → IMMEDIATELY set youtube_query to a specific search string. Do it NOW, in this reply, not after asking permission.
- If the user says "yep", "yes", "ok", "sure" after you mentioned a video → set youtube_query immediately.
- NEVER say "I'll find a video for you" or "That's settled!" without actually setting youtube_query in the same reply. That is a broken experience.
- NEVER ask "Want to watch a video?" — just show it.
- If no video is needed, set youtube_query to an empty string "".
- Good examples: user says "show me a biryani video" → set youtube_query to "chicken biryani recipe step by step tutorial". User says "how do I center a div" → optionally set youtube_query to "how to center div CSS tutorial".
...
YOU MUST ALWAYS RESPOND IN THIS EXACT JSON FORMAT — no markdown, no explanation outside the JSON:
{
  ...
  "youtube_query": "specific YouTube search query if a video would help, otherwise empty string"
}"""
    if last_img_prompt:
    ...
    for attempt in range(2):
        try:
            ...
            return {
                ...
                "youtube_query": parsed.get("youtube_query", "").strip(),
            }
        except Exception:
            if attempt == 1:
...
- Rakai is fully open-source! The GitHub repo is: https://github.com/heyitsaadin/Rekai-ai
- If a user asks how you were built, how Aadin made you, can they see the code, or anything about your source code → enthusiastically share the GitHub link and invite them to explore it.
- Example: "Aadin open-sourced me! 🎉 Check out the full code: https://github.com/heyitsaadin/Rekai-ai — star it if you like it! ⭐"

YOUTUBE VIDEO CARD RULES:
- If the user asks for a video, tutorial, or wants to watch/learn something → IMMEDIATELY append [YOUTUBE:specific search query] at the end of your reply. Do it in THIS reply, not after asking.
- If the user says "yep", "yes", "ok", "sure" in response to you mentioning a video → include [YOUTUBE:...] in THIS reply.
- NEVER say "I'll find a video" or promise a video without actually including [YOUTUBE:...] in the same reply. That is a broken experience.
- NEVER ask "Want to watch a video?" — just show it directly.
- Only ONE [YOUTUBE:...] per reply. Skip it entirely if no video is relevant."""
    clean_msgs, last_img_prompt, last_img_analysis = _build_clean_history(history)
...
def _build_reply(user_msg):
    ...
    youtube_query = decision.get("youtube_query", "").strip()
    if action == "text":
        ...
        if "##IMAGE:" in raw_reply:
            ...
            return reply
        # Append YouTube tag so frontend can render the card
        if youtube_query:
            if not raw_reply.strip():
                raw_reply = "Here's a video that should help! 🎬"
            raw_reply = raw_reply.rstrip() + f"\n[YOUTUBE:{youtube_query}]"
        return raw_reply
