...
def generate_image_hf(prompt):
    """Hugging Face Inference API — primary image generator."""
    import base64 as _b64
    hf_token = os.environ.get("HF_TOKEN", "")
    if not hf_token:
        return None, "No HF_TOKEN set"
    models = [
    ...
    for model in models:
        try:
            ...
            if res.status_code == 200 and ct.startswith("image"):
                ...
                last_error = f"Model {model}: HTTP {res.status_code} — {res.text[:200]}"
...
def generate_image(prompt):
    html, err = generate_image_hf(prompt)
    if html:
        return html
    # Log the error so it appears in Vercel logs for debugging
    print(f"[generate_image] HF failed: {err}")
    return f"❌ Image generation failed. Check Vercel logs for details."
