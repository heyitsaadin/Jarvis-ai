...
- If a user is struggling, confused, explicitly asks for a video/tutorial, or says "yep" / "yes" after you offered a video → set youtube_query to a specific search string e.g. "chicken biryani recipe step by step" or "python for loops beginner tutorial".
- If no video is needed, set youtube_query to an empty string "".
- NEVER write [YOUTUBE:...] in the reply text. Use the youtube_query JSON field instead.
- Do NOT ask "Want to watch a video?" in the reply — just set youtube_query directly when appropriate.
...
YOUTUBE VIDEO CARD RULES:
- If a user is struggling, confused, explicitly asks for a video/tutorial, or says "yep"/"yes" after you offered a video → append [YOUTUBE:specific search query] at the very end of your reply text.
- Example: "Here's how it works! ... [YOUTUBE:python list comprehension tutorial]"
- Only ONE [YOUTUBE:...] tag per reply. Only when a video genuinely helps — not for every message."""
    clean_msgs, last_img_prompt, last_img_analysis = _build_clean_history(history)
