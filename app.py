...
def wikipedia_search(query):
    ...
    try:
        ...
        # If no results, try with shorter query (first 3 words)
        if not results:
            short_q = " ".join(query.split()[:3])
            if short_q != query:
                results = _search(short_q)
...
def _build_reply(user_msg):
    ...
    if user_msg.lower().startswith('/search '):
        ...
        if result:
            return result
        return "❌ No results found for that. Try a different query or be more specific."
    ...
    if action == "web_search":
        ...
        if result:
            return result
        return "I couldn't find any information on that. Try /search <query> to search directly."
