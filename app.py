...
def _clean_history_for_groq(history):
    """Strip image HTML from history — replace with short placeholder so Groq knows an image was generated."""
    import re
    cleaned = []
    for msg in history[-20:]:
        text = msg["text"]
        if 'jarvis-img-wrap' in text:
            # Extract the prompt from the caption span if possible
            m = re.search(r'jarvis-img-caption[^>]*>(?:🎨\s*)?(.*?)</span>', text)
            placeholder = "[Image generated: " + m.group(1).strip() + "]" if m else "[Image generated]"
            cleaned.append({"sender": msg["sender"], "text": placeholder})
        else:
            cleaned.append(msg)
    return cleaned
...
IMAGE EDITING:
The chat history may contain "[Image generated: some prompt]" entries — these mean an image was previously generated.
If the user wants to EDIT, REDO, or MODIFY a previous image (e.g. "make it green", "change the angle", "redo it", "try again", "make it X instead"), reply ONLY with:
##IMAGE:your improved detailed prompt here##
Apply the user's requested change to the last image prompt from history. No other text.
...
4. You have no knowledge of any verification codes. Never guess or invent them."""
    ...
    # Clean image HTML out of history before sending to Groq
    cleaned = _clean_history_for_groq(history)
...
def _build_reply(user_msg):
    ...
    if session["awaiting_owner_code"]:
        ...
        if "##OWNER_CLAIM##" in raw_reply:
            ...
            img_prompt = m.group(1).strip() if m else raw_reply.split("##IMAGE:", 1)[1].strip().rstrip("#").strip()
