...
def update_username():
    ...
    cur = conn.cursor()
    old = session["user"]
    cur.execute("UPDATE users    SET username=%s WHERE username=%s", (new_username, old))
    cur.execute("UPDATE chats    SET username=%s WHERE username=%s", (new_username, old))
    cur.execute("UPDATE sessions SET username=%s WHERE username=%s", (new_username, old))
    cur.execute("UPDATE history  SET username=%s WHERE username=%s", (new_username, old))
    conn.commit(); cur.close(); conn.close()
...
def update_password():
    ...
    if not current or not new_pw:
        return jsonify({"error": "Fill in both fields"})
    if len(new_pw) < 4:
        return jsonify({"error": "New password must be at least 4 characters"})
...
def get_sessions():
    if "user" not in session:
        return jsonify({"error": "not logged in"}), 401
    rows = get_user_sessions(session["user"])
