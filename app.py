...
def _build_clean_history(history):
    ...
    # ── Pass 2: only take the last 6 messages (3 turns) — always most recent ──
    # This hard cap prevents context overflow on llama-3.1-8b-instant (8k tokens).
    # Image analysis/edit markers are excluded from the window count since they
    # are injected into the system prompt separately.
    WINDOW = 6
    recent = history[-WINDOW * 2:]  # grab a wider slice to account for filtered entries
    clean_msgs = []
    for msg in recent:
        txt = msg["text"]
        if txt.startswith("[IMAGE GENERATED:"):
            ...
            entry = {"role": role, "content": txt[:150]}
        else:
            role = "user" if msg["sender"] == "You" else "assistant"
            # Hard cap per message: 400 chars is plenty for context
...
{
  "action": "text" | "get_time" | "get_date" | "math" | "quiz_redirect" | "generate_image" | "edit_image",
...
def _build_reply(user_msg):
    ...
    if action == "math":
        ...
        if not expr:
            # Fallback: try the raw user message
            expr = user_msg
        try:
            ...
            # Brain thought it was math but safe_eval disagrees → fall through to text
            pass
