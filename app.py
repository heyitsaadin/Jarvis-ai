...
def _flush_chat_session():
    ...
    try:
        ...
        if user and key and len(msgs) > 1:
...
def send():
    ...
    # Auto-save chat every 10 messages
    if len(session["messages"]) % 10 == 0:
        threading.Thread(
            target=save_chat_session,
            args=(session["user"], session.get("chat_key"), list(session["messages"])),
            daemon=True
        ).start()
...
def account_chats():
    ...
    chats = [{"id": r["id"], "session_key": r["session_key"],
               "created_at": r["created_at"], "updated_at": r["updated_at"],
               "preview": (_json_mod.loads(r["messages"] or "[]") + [{}])[0].get("text", "")[:60]}
             for r in rows]
