...
TOPIC_KEYWORDS = {
    ...
    "tech":     ["ai","machine learning","neural","model","gpt","chatgpt","rekai","llm","data","server","cloud"],
...
def get_greeting(username):
    ...
    greetings = [
        ...
        f"Good {period}, {username}! ✨ Great to see you — what can Rekai what can Rekai today?",
        f"Hey hey, {username}! 🌟 Good {period}! I'm all ears — what do you need?",
        f"Good {period}, {username}! 🤖 Rekai online and ready. What's up?",
    ]
...
def generate_image(prompt):
    ...
    return (
        f'<div class="rekai-img-wrap">'
        f'<img src="{url}" alt="{prompt}" class="rekai-img" '
        f'onload="this.classList.add(\'loaded\')" '
        f'onerror="this.parentElement.innerHTML=\'❌ Could not generate image. Try a different prompt.\'">'
        f'<div class="rekai-img-footer">'
        f'<span class="rekai-img-caption">🎨 {prompt}</span>'
        f'<button onclick="downloadImage(\'{url}\',\'{escaped_prompt}\')" class="rekai-img-dl">'
        f'⬇ Download'
        f'</button>'
        f'</div>'
        f'</div>'
...
def generate_image_nvidia(prompt):
    ...
    try:
        ...
        if "data" in data and data["data"]:
            ...
            html = (
                f'<div class="rekai-img-wrap">'
                f'<img src="{src}" alt="{prompt}" class="rekai-img" '
                f'onload="this.classList.add(\'loaded\')" '
                f'onerror="this.parentElement.innerHTML=\'❌ Could not generate image. Try a different prompt.\'">'
                f'<div class="rekai-img-footer">'
                f'<span class="rekai-img-caption">🎨 {prompt} ✨</span>'
                f'<button onclick="downloadImage(\'{src}\',\'{escaped_prompt}\')" class="rekai-img-dl">'
                f'⬇ Download'
                f'</button>'
                f'</div>'
                f'</div>'
...
def _compact_for_session(text):
    if 'rekai-img-wrap' in text:
        m = re.search(r'rekai-img-caption[^>]*>🎨\s*(.*?)(?:\s*✨\s*)?</span>', text)
...
def _build_clean_history(history):
    ...
    for msg in history:
        ...
        elif 'rekai-img-wrap' in txt:
            m = re.search(r'rekai-img-caption[^>]*>🎨\s*(.*?)(?:\s*✨\s*)?</span>', txt)
...
def ask_rekai_brain(prompt, history=None):
    if history is None:
        history = []
    API_KEY = os.environ.get("GROQ_API_KEY")
    if not API_KEY:
    ...
    system_msg = """You are Rekai, a personal AI assistant created by Aadin. You have a warm, friendly, and engaging personality — like a smart best friend who genuinely enjoys helping.
...
def ask_rekai(prompt, history=None, wants_code=False):
    if history is None:
        history = []
    API_KEY = os.environ.get("GROQ_API_KEY")
    if not API_KEY:
...
- NEVER use code blocks for recipes, instructions, or any non-code content."""
    system_msg = f"""You are Rekai, a personal AI assistant created by Aadin. You have a warm, friendly, and engaging personality — like a smart best friend who genuinely enjoys helping.
...
def _build_reply(user_msg):
    ...
    if contains_bad_words(user_msg):
        send_discord_alert(session["user"], "Abusive language toward Rekai or Aadin", user_msg)
    ...
    _has_recent_image = any(
        'rekai-img-wrap' in m.get("text", "") or m.get("text", "").startswith("[IMAGE GENERATED:")
    ...
    if _is_img_question:
        ...
        while _waited < 8:
            ...
            time.sleep(0.5)
            _waited += 0.5
    decision = ask_rekai_brain(user_msg, session["messages"])
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            session["messages"] = [{"sender": "Rekai", "text": greeting}]
...
def send():
    ...
    session["messages"].append({"sender": "Rekai", "text": session_text})
    update_profile(session["user"], reply, "Rekai")
...
def generate_image_route():
    ...
    session["messages"].append({"sender": "Rekai", "text": _compact_for_session(reply)})
    update_profile(session["user"], reply, "Rekai")
    _msgs_copy = list(session["messages"])
    threading.Thread(
        target=_auto_analyse_generated_image,
...
def send_image():
    ...
    try:
        ...
        session["messages"].append({"sender": "Rekai", "text": stored_reply})
        update_profile(session["user"], user_label, "You")
        update_profile(session["user"], reply, "Rekai")
        session.modified = True
...
def account_chats():
    ...
    for r in rows:
        ...
        if not preview:
            for m in msgs:
                raw = m.get("text", "")
                if (raw.startswith("[IMAGE ANALYSIS") or "rekai-img-wrap" in raw
...
def account_delete_chat(session_key):
    ...
    if session.get("chat_key") == session_key:
        session["messages"] = [{"sender": "Rekai", "text": get_greeting(session["user"])}]
        session["chat_key"] = secrets.token_hex(8)
        session.modified = True
    return jsonify({"ok": True}), 200
...
def share_chat():
    ...
    for m in msgs:
        ...
        if txt.startswith("[IMAGE GENERATED:"):
            ...
            txt = f'<div class="rekai-img-wrap"><img src="{img_url}" alt="{prompt_text}" class="rekai-img" onload="this.classList.add(\'loaded\')"><div class="rekai-img-footer"><span class="rekai-img-caption">🎨 {prompt_text} ✨</span></div></div>'
...
def admin_analyse_chats(username):
    ...
    sys_p = """You are an AI analyst reviewing aggregated usage metadata about a user of a chatbot called Rekai.
