...
5. NEVER share YouTube links, video URLs, or any media links — you cannot browse the internet. If a user asks for a video, just say the app will find one for them automatically.
...
_YT_TRIGGERS = [
    "video", "videos", "vedio", "vedios",   # common typo too
    "tutorial", "tutorials", "watch", "youtube",
    "show me how", "how to video", "video tutorial",
    "teach me", "walk me through", "step by step video",
    "clip", "recording",
]

def _detect_youtube_request(user_msg):
    """
    Returns a YouTube search query string if the message is clearly
    asking for a video, otherwise returns empty string.
    Pure Python — no Groq involved.
    """
    msg = user_msg.lower().strip()
    if not any(t in msg for t in _YT_TRIGGERS):
        return ""
    # Strip filler words to build a clean search query
    query = user_msg.strip()
    for filler in ["can you", "could you", "please", "show me", "find me",
                   "give me", "i want", "i need", "get me", "a ", "an ",
                   "some ", "the "]:
        query = query.lower().replace(filler, " ")
    query = " ".join(query.split())  # collapse whitespace
    # Replace common typos
    query = query.replace("vedio", "video").replace("vedios", "videos")
    # If query got too short, fall back to original message
    if len(query) < 5:
        query = user_msg.strip()
    return query[:120]  # cap length
...
def _build_reply(user_msg):
    ...
    # YouTube detection is handled entirely in Python — Groq is not involved
    youtube_query = _detect_youtube_request(user_msg)
    if action == "text":
